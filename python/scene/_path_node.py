...
from ._path import (
    DrawMesh, StrokeStyle, build_path_meshes, commands_snapshot,
...
class Path(Node):
    ...
    def _ensure_flattened(self, raster_scale=1.0):
        if self._flattened_subpaths is None or self._geometry_dirty:
            tol = self.curve_tolerance / max(1.0, float(raster_scale))
    ...
    def _rebuild_texture(self, renderer, raster_scale):
        subpaths = self._ensure_flattened(raster_scale)
        bounds = self._local_bounds
        if not subpaths or bounds is None:
        ...
        need_fill = self._cached_fill_tris is None or self._raster_dirty
        if need_fill:
            ...
            fill_meshes, stroke_meshes = build_path_meshes(
                ...
                stroke_style=stroke_style, fill_rule=self.fill_rule, offset=(dx, dy),
            )
            self._cached_fill_tris = fill_meshes
        else:
            _, stroke_meshes = build_path_meshes(
                subpaths, fill_color=None, stroke_color=stroke_color,
                stroke_style=stroke_style, fill_rule=self.fill_rule, offset=(dx, dy),
            )
            fill_meshes = self._cached_fill_tris

        meshes = fill_meshes + stroke_meshes
        if not meshes:
            if self._tex is not None:
                self._tex.close()
                self._tex = None
        ...
        pw = max(1, int(round(width * renderer.screen_scale * raster_scale)))
        ph = max(1, int(round(height * renderer.screen_scale * raster_scale)))
        if self._tex is None or self._path_size != (width, height) or abs(self._rscale - raster_scale) > 0.249:
            if self._tex is not None:
                self._tex.close()
            self._tex = Texture.render_target(pw, ph)
        renderer.render_color_meshes(meshes, clear_color=(0, 0, 0, 0), target_texture=self._tex, viewport=(width, height))
    ...
    def _ensure_texture(self, renderer, raster_scale):
        if self._tex is None or self._raster_dirty or self._stroke_dirty or abs(self._rscale - raster_scale) > 0.249:
    ...
    def _get_meshes(self, renderer):
        """Return list of DrawMesh in path-local space for direct rendering.

        Cached: returns the same list if path_version hasn't changed.
        """
        ver = self._path_version
        cached = getattr(self, '_cached_meshes', None)
        if cached is not None and getattr(self, '_cached_mesh_version', -1) == ver:
            return cached
        subpaths = self._ensure_flattened(1.0)
        if not subpaths or self._local_bounds is None:
            self._raster_dirty = False
            self._stroke_dirty = False
            self._cached_meshes = []
